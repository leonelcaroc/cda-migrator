...
export function resolveCoopType(type, coopName) {
    ...
    const multipurposeType = coopTypeMap.find((t) => t.name.toLowerCase() === "multipurpose");
    const federationType = coopTypeMap.find((t) => t.name.toLowerCase() === "federation");
    const cooperativeBankType = coopTypeMap.find((t) => t.name.toLowerCase() === "cooperative bank");
    ...
    const rawTypes = type
        ...
        .map((t) => t.trim())
        .filter(Boolean);
    const hasMultipurposeRawType = rawTypes.some((t) => /multi[\s-]?purpose/i.test(t));
    const matchedTypes = rawTypes
        .map((raw) => coopTypeMap.find((t) => t.name.toLowerCase() === raw.toLowerCase()))
        .filter(Boolean);
    const isMultipurpose = hasMultipurposeRawType ||
...
export function normalizeDate(dateStr) {
    if (dateStr == null)
        return null; // covers null and undefined
    const value = String(dateStr).trim(); // <-- convert first, then trim
    if (!value)
        return null; // empty string
    if (/^(0+[-/]?){2,3}0+$/.test(value))
        return null; // all-zero placeholders
    let day, month, year;
