...
/**
 * Get the next available error log file path with versioning
 */
function getErrorLogFilePath(): string {
  const basePath = path.join(process.cwd(), "migration_errors_log.txt");
...
/**
 * Initialize the error log file - creates new versioned file
 */
function initializeErrorLog(): void {
  try {
    // Check if file exists (shouldn't with versioning, but just in case)
